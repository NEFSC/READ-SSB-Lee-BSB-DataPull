...
#          File:   data_folder/main/commercial/hake_sizes_{vintage_string}.Rds
#                  (grade/market keyfile; produced by hake_price_categories.R)
...
#
# No year-range filter is applied — the full history of hake commercial
...
# the landed-to-live weight conversion ratio (cf_lndlb_livlb, in the hake_sizes
...
# all market catgories for hake have live pounds=landed pounds
...
hake_sizes <- readRDS(here("data_folder", "main", "commercial",
...
landings_all <- landings_all %>%
  left_join(hake_sizes, by = join_by(itis_tsn, grade_code, market_code))
...
# If this fires, hake_sizes has duplicate keys — investigate before proceeding.
...
message(glue("Unmatched to hake_sizes (merge_species_codes == 1): ",
