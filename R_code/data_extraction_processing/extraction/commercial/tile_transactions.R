# ################################################################################
# Script:  tile_transactions.R
# Purpose: Extract transaction-level commercial Tilefish landings from
...
#          File:   data_folder/main/commercial/tile_sizes_{vintage_string}.Rds
#                  (grade/market keyfile; produced by tile_price_categories.R)
# Outputs: data_folder/main/commercial/tile_landings_all_{vintage_string}.Rds
#          Transaction-level tilefish commercial landings with prices and
...
where cl.itis_tsn in ({codes_sql})
     and cl.rec = 0"
)
...
# the landed-to-live weight conversion ratio (cf_lndlb_livlb, in the tile_sizes
...
# all market catgories for tile have live pounds=landed pounds
...
# ################################################################################
# Section 4: Merge in species/market-category keyfile (tile_sizes)
# ################################################################################

# tile_sizes is the species-grade-market lookup table produced by
# tile_price_categories.do (R equivalent: tile_price_categories.R when ported).
# Fields added: nespp4, grade_desc, market_desc, cf_lndlb_livlb.
#

tile_sizes <- readRDS(here("data_folder", "main", "commercial",
...
landings_all <- landings_all %>%
  left_join(tile_sizes, by = join_by(itis_tsn, grade_code, market_code))
...
# If this fires, tile_sizes has duplicate keys — investigate before proceeding.
...
output_file <- glue("tile_landings_all_{vintage_string}.Rds")
...
message(glue("Unmatched to tile_sizes (merge_species_codes == 1): ",
